...
export class ListaAlertasComponent implements OnInit {
  ...
  niveles: NivelAlerta[] = ['Verde', 'Amarilla', 'Naranja', 'Roja'];
  ...
  cargar(): void {
    ...
    this.alertaService.listar().subscribe({
      ...
      error: () => { this.error = 'Error al cargar alertas'; this.cargando = false; },
    });
  }
  ...
  filtrarPorNivel(): void {
    if (this.filtroNivel) {
      this.alertaService.listarPorNivel(this.filtroNivel).subscribe({
  ...
  guardar(): void {
    ...
    if (this.modoEdicion && this.form.id) {
      ...
      this.alertaService.actualizar(this.form.id, this.form)
        .subscribe({

          next: (alertaActualizada) => {

            const index = this.alertas.findIndex(a => a.id === alertaActualizada.id);
      ...
      this.alertaService.crear(this.form)
        .subscribe({
  ...
  formVacio(): Alerta {
    return { nivel: 'Verde', mensaje: '', idUsuario: 1, idEvento: 0 };
  }
}
